...
int main(int argc, char** argv) {
    ...
    if (config.save_point_cloud) {
        ...
        if (pipeline.exportPointCloud(output_file)) {
            ...
            std::cerr << "Error: Failed to export point cloud." << std::endl;
        }
    }
    
    if (config.save_mesh) {
        std::string mesh_file = config.output_dir;
        if (mesh_file.find(".obj") == std::string::npos) {
            mesh_file += "/mesh.obj";
        }
        
        std::cout << "\nExporting mesh to " << mesh_file << "..." << std::endl;
        if (pipeline.exportMesh(mesh_file)) {
